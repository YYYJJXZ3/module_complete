...
function appendD1() {
    $.getJSON("js/provinceData.json", function (data) {
...
function appendD2() {
    ...
    $.getJSON("js/provinceData.json", function (data) {
        ...
        $jsontip.empty();//清空内容
        $.each(newdata, function (infoIndex, info) {
            strHtml += '<tr><td><b class="roleNum">' + (infoIndex + 1) + '</b></td><td>' + info.col0 + '</td><td>' + info.col2 + '</td></tr>';
        })
        $jsontip.html(strHtml);//显示处理后的数据
...
function appendD3() {
    $.getJSON("js/provinceData.json", function (data) {
        ...
        $jsontip.empty();//清空内容
        $.each(newdata, function (infoIndex, info) {
            strHtml += '<tr><td><b class="roleNum">' + (infoIndex + 1) + '</b></td><td>' + info.col0 + '</td><td>' + info.col3 + '</td></tr>';
        })
        $jsontip.html(strHtml);//显示处理后的数据
...
function appendD4() {
    $.getJSON("js/provinceData.json", function (data) {
        ...
        $jsontip.empty();//清空内容
        $.each(newdata, function (infoIndex, info) {
            strHtml += '<tr><td><b class="roleNum">' + (infoIndex + 1) + '</b></td><td>' + info.col0 + '</td><td>' + info.col4 + '</td></tr>';
        })
        $jsontip.html(strHtml);//显示处理后的数据
