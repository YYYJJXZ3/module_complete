	...
	$.jstree.plugin("dnd", {
		...
		_fn : {
			...
			dnd_show : function () {
				...
				if(r === false) { $.vakata.dnd.helper.children("ins").attr("class","jstree-ok"); } //拖拽失效修改处
...
$.vakata.xslt = function (xml, xsl, callback) {
		var r = false, p, q, s;
		// IE9
		if(r === false &&$.support.leadingWhitespace){// window.ActiveXObject) {
			try {
	...
	var xsl = {
		'nest' : '<' + '?xml version="1.0" encoding="utf-8" ?>' + 
			...
			'				</xsl:for-each>' + 
			'					<ins>' + 
			'						<xsl:attribute name="class"><xsl:value-of select="@ico" /> ' + 
