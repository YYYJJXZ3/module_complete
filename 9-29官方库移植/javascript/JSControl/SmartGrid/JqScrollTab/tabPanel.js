...
TabPanel.prototype = {
  ...
  addTab : function(tabitem){
    ...
    else if(this.scrollFinish)
    {
      ...
      divtitle.text(tabitem.title);
      divtitle.appendTo(tab);

	    var wFix = tabitem.closable==false ? 0 : 5;
      if(tabitem.icon) {
          divtitle.addClass('icon_title');
          divtitle.css('background-image', 'url("' + tabitem.icon + '")');
          if (divtitle.width() > (this.tabWidth - 35 - wFix)) {
              divtitle.width((this.tabWidth - 50 - wFix));
              divtitle.attr('title', tabitem.title);
          tab.append('<DIV>...</DIV>');
        }
      } else {
          divtitle.addClass('title');
          if (divtitle.width() > (this.tabWidth - 19 - wFix)) {
              divtitle.width((this.tabWidth - 30 - wFix));
              divtitle.attr('title', tabitem.title);
          tab.append('<DIV>...</DIV>');
        }
      }
  ...
  kill : function(position){
    ...
    this.tabs[position].divtitle.remove();
  ...
  setTitle : function(position,title){
    position = this.getTabPosision(position);
    if(position < this.tabs.length)
      this.tabs[position].divtitle.text(title);
  },
  ...
  getTitle : function(position){
    position = this.getTabPosision(position);
    return this.tabs[position].divtitle.text();
  },
  ...
  setDisable : function(position,disable){
    ...
    if(position < this.tabs.length){
      ...
      if(disable){
        ...
        this.tabs[position].divtitle.addClass('.disabled');
      }else{
        this.tabs[position].tab.removeAttr('disabled');
        this.tabs[position].divtitle.removeClass('.disabled');
      }
    }
  },
