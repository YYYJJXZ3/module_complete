...
function GooFlow(bgDiv, property) {
    ...
    if (navigator.userAgent.indexOf("MSIE 10.0") > 0 || navigator.userAgent.indexOf("MSIE 9.0") > 0 || navigator.userAgent.indexOf("MSIE 8.0") > 0 || navigator.userAgent.indexOf("MSIE 7.0") > 0 || navigator.userAgent.indexOf("MSIE 6.0") > 0) {
        GooFlow.prototype.useSVG = "";
        if (navigator.userAgent.indexOf("MSIE 10.0") > 0 && (mode == 9 || mode == 10 || mode == 11)) {
            GooFlow.prototype.useSVG = "1";
        } else if (navigator.userAgent.indexOf("MSIE 9.0") > 0 && (mode == 9 || mode == 10 || mode == 11)) {
            GooFlow.prototype.useSVG = "1";
        } else if (navigator.userAgent.indexOf("MSIE 8.0") > 0 && (mode == 9 || mode == 10 || mode == 11)) {
            GooFlow.prototype.useSVG = "1";
        }
        else if (navigator.userAgent.indexOf("MSIE 7.0") > 0 && (mode == 9 || mode == 10 || mode == 11)) {
            GooFlow.prototype.useSVG = "1";
        }
    }
    ...
    if (property.isShowPager != undefined && property.isShowPager == true) {
        this.isShowPager = true;
    }
    var tmp = "";
    if (property.haveHead) {
        tmp = "<div class='GooFlow_head'><label title='" + (property.initLabelText || "空流程图") + "'>" + (property.initLabelText || "空流程图") + "<\/label>"
        for (var x = 0; x < property.headBtns.length; ++x) {
            if (property.headBtnsText != undefined) {
                tmp += "<a href='javascript:void(0)'  title='" + property.headBtnsText[x] + "'  class='GooFlow_head_btn'><b class='ico_" + property.headBtns[x] + "'></b></a>"
            } else {
    ...
    this.initWorkForNode(property);//放到这里是配置和呈现界面都有点击事件
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            //e.data.inthis.$max++;
            //---------yxy modify 2014 06 13-------------
            var node_id = tools.CRC32(e.data.inthis.$id + "_node_" + e.data.inthis.$max);
            var nodeName = e.data.inthis.$nowType == 'start' ? "开始" : "node_" + e.data.inthis.$max;
            e.data.inthis.addNode(node_id, {
                id: node_id, name: nodeName, left: X, top: Y, type: e.data.inthis.$nowType,
                NodeInfo: demoNode.NodeInfo
            });
            e.data.inthis.$max++;
        });
        //划线时用的绑定
        ...
        //this.initWorkForNode();//呈现界面也要有注册事件
        ...
        //用户自定义事件
        this.onNewNodeDBClick = null;//节点双击事件 yxy add
        //this.onNewLineDBClick = null;//连线双击事件 yxy add
        this.onNewNodeClick = null;//节点单击事件s
...
GooFlow.prototype = {
    ...
    initDraw: function (id, width, height) {
        ...
        if (this.$editable) {
            ...
            $(this.$draw).delegate(tmpClk, "dblclick", { inthis: this }, function (e) {
                ...
                x = (parseInt(from[0], 10) + parseInt(to[0], 10)) / 2 - 60;
                y = (parseInt(from[1], 10) + parseInt(to[1], 10)) / 2 - 12;
                //if (e.data.inthis.onNewLineDBClick == null) {
                //    var t = getElCoordinate(This.$workArea[0]);
                //    This.$textArea.val(oldTxt).css({
                //        display: "block", width: 120, height: 14,
                //        left: t.left + x - This.$workArea[0].parentNode.scrollLeft,
                ...
                //    }).data("id", This.$focus).focus();
                //} else {
                //    var id = This.$lineData[this.id].id;
                //    alert("连线被双击了" + oldTxt + "," + x + "," + y + "," + from + "," + to + "," + id);
                //}
    ...
    addNode: function (id, json) {
        ...
        if (json.type.indexOf(" round") < 0) {
            ...
            if (json.type != "node") {
                this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr><td class='ico'><b class='ico_" + json.type + "'></b></td><td title='" + json.title + "'>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
            } else {
                this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' style='border:0;background:Transparent; box-shadow:none;top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='0' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'>" +
                 "<tr><td class='ico' style='border:0px solid blue;width:5px;'>&nbsp;</td><td style='padding-left:0; border:0px solid red; margin-left:10px;'>" +
                  "<div id=\"lingxing_top_" + id + "\" class='lingxing_top'></div>" +
                 "<div id=\"lingxing_bottom_" + id + "\" class='lingxing_bottom'></div>" +
                 //"<span id=\"nodeName_" + id + "\" title='" + json.title + "' style='border:0px solid blue;position:absolute;top:-" + (json.height - 5) + "px;left:0px; '>" + json.name + "</span></td></tr>" +
                 "<span id=\"nodeName_" + id + "\" title='" + json.title + "'>" + json.name + "</span></td></tr>" +
                 "</table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
                var h = json.height - 2;
                if (h < 24) {
                    h = 24;
                }
                ...
                var nodeH = h < 30 ? 6 : (h / 5);
                var nodeW = w < 90 ? 35 : (w / 20);
                this.$nodeDom[id].find("span").css({ "position": "absolute", "top": "" + nodeH + "px", "left": "" + nodeW + "px" });
                this.$nodeDom[id].find(".lingxing_top").css({ "border-bottom": "" + h / 2 + "px solid #FC8F3C", "border-left": "" + (w / 2) + "px solid Transparent", "border-right": "" + (w / 2) + "px solid Transparent" });
                this.$nodeDom[id].find(".lingxing_bottom").css({ "border-top": "" + h / 2 + "px solid #FC8F3C", "border-left": "" + (w / 2) + "px solid Transparent", "border-right": "" + (w / 2) + "px solid Transparent" });
    ...
    initWorkForNode: function (property) {
        //绑定点击事件
        this.$workArea.delegate(".GooFlow_item", "click", { inthis: this }, function (e) {
            if (typeof property.click == "function") {
                var id = $(this).attr("id");
                $.each(e.data.inthis.$nodeData, function (i, item) {
                    if (item.type != "start" && item.type != "node" && item.id == id) {
                        property.click(item);
                        return false;
                    }
                });
        ...
        if (this.isShowPager != true) {
            ...
            this.$workArea.delegate(".GooFlow_item", "mouseup", { inthis: this }, function (e) {
                ...
                //--------yxy modify 2014 06 13------------
                var line_id = tools.CRC32(This.$id + "_line_" + This.$max);
                if (lineStart) This.addLine(line_id, { id: line_id, from: lineStart.id, to: this.id, name: "", mark: false });
                This.$max++;
            });
        ...
        this.$workArea.delegate(".ico + td", "dblclick", { inthis: this }, function (e) {
            ...
            if (e.data.inthis.onNewNodeDBClick != null && This.isShowPager == true) {
                //var oldTxt = this.innerHTML;
                //var id = $(this).parents(".GooFlow_item").attr("id");
                //var t = getElCoordinate(This.$workArea[0]);
                //This.$textArea.val(oldTxt).css({
                ...
                //    top: t.top + 2 + This.$nodeData[id].top - This.$workArea[0].parentNode.scrollTop
                //}).data("id", This.$focus).focus();
                if (nodeType == 'task' || nodeType == 'chat' || nodeType == "node") {
                    e.data.inthis.onNewNodeDBClick(id, nodeName, nodeType);
                }
            } else {
                if (nodeType == "chat" || nodeType == "task" || nodeType == "node") {
                    if (nodeType == "node") {
                        nodeName = $("#nodeName_" + id).text();
                    }
                    //节点属性信息
                    nodePropertyInfo(id, nodeName, nodeType, This.$nodeData[id].left, This.$nodeData[id].top);
                }
                else {//其他节点
                    var oldTxt = this.innerHTML;
                    var t = getElCoordinate(This.$workArea[0]);
        ...
        this.$workArea.delegate(".GooFlow_item > div > div[class!=rs_close]", "mousedown", { inthis: this }, function (e) {
            ...
            document.onmousemove = function (e) {
                ...
                isMove = true;
                switch (cursor) {
                    case "nw-resize": This.$ghost.css({ width: X - 2 + "px", height: Y - 2 + "px" });
                        if (This.$nodeData[id].type == 'node') {
                            var w = X - 2;
                            var h = Y - 2;
                            $("#nodeName_" + This.$nodeData[id].id).css({ "position": "absolute", "top": "" + (h / 2) + "px", "left": "" + (w / 5) + "px" });
                            $("#lingxing_top_" + This.$nodeData[id].id).css({ "border-bottom": "" + (h / 2) + "px solid #FC8F3C", "border-left": "" + (w / 2) + "px solid Transparent", "border-right": "" + (w / 2) + "px solid Transparent" });
                            $("#lingxing_bottom_" + This.$nodeData[id].id).css({ "border-top": "" + (h / 2) + "px solid #FC8F3C", "border-left": "" + (w / 2) + "px solid Transparent", "border-right": "" + (w / 2) + "px solid Transparent" });
                        }
                        break;
                    case "w-resize": This.$ghost.css({ width: X - 2 + "px" });
                        var w = X - 2;
                        $("#nodeName_" + This.$nodeData[id].id).css({ "top": "" + (Y / 2) + "px", "left": "" + (w / 2) + "px" });
                        $("#lingxing_top_" + This.$nodeData[id].id).css({ "border-bottom": "" + (Y / 2) + "px solid #FC8F3C", "border-left": "" + (w / 2) + "px solid Transparent", "border-right": "" + (w / 2) + "px solid Transparent" });
                        $("#lingxing_bottom_" + This.$nodeData[id].id).css({ "border-top": "" + (Y / 2) + "px solid #FC8F3C", "border-left": "" + (w / 2) + "px solid Transparent", "border-right": "" + (w / 2) + "px solid Transparent" });
                        break;
                    case "n-resize": This.$ghost.css({ height: Y - 2 + "px" });
                        var h = Y - 2;
                        $("#nodeName_" + This.$nodeData[id].id).css({ "top": "" + (h / 2) + "px", "left": "" + (X / 2) + "px" });
                        $("#lingxing_top_" + This.$nodeData[id].id).css({ "border-bottom": "" + (h / 2) + "px solid #FC8F3C", "border-left": "" + (X / 2) + "px solid Transparent", "border-right": "" + (X / 2) + "px solid Transparent" });
                        $("#lingxing_bottom_" + This.$nodeData[id].id).css({ "border-top": "" + (h / 2) + "px solid #FC8F3C", "border-left": "" + (X / 2) + "px solid Transparent", "border-right": "" + (X / 2) + "px solid Transparent" });
                        break;
                }
            }
            document.onmouseup = function (e) {
    ...
    blurItem: function () {
        if (this.$focus != "") {
            ...
            if (jq.prop("tagName") == "DIV") {
                ...
                if (this.$nodeData[this.$focus].type == "node") {
                    jq.css({
                        'position': 'absolute', 'top': '' + this.$nodeData[this.$focus].top + 'px', 'left': '' + this.$nodeData[this.$focus].left + 'px',
                        'background': '0', 'border': 'Transparent',
                        'border-radius': '3px', 'color': '#15428B', 'background-color': 'Transparent',
                        'box-shadow': 'none', '-webkit-box-shadow': '1px 1px 2px rgba(148,170,194,2)', '-moz-box-shadow': '1px 1px 2px rgba(148,170,194,2)'
                    });
                }
    ...
    focusItem: function (id, bool) {
        ...
        if (jq.prop("tagName") == "DIV") {
            ...
            if (this.$nodeData[id].type == "node") {
                jq.css({
                    'position': 'absolute', 'top': '' + this.$nodeData[id].top + 'px', 'left': '' + this.$nodeData[id].left + 'px',
                    'background': '#DDE7F4 url(img/gooflow_btn_bg.png) repeat-x 0px -72px', 'border': '#7DA2CE solid 1px',
    ...
    setName: function (id, name, type) {
        ...
        if (type == "node") {//如果是结点
            ...
            if (this.$nodeData[id].type != "node") {
                ...
                this.$nodeDom[id].find("td:eq(1) span").text(name);
            }
            var hack = 0;
            if (navigator.userAgent.indexOf("8.0") != -1) hack = 2;
            var width;
            var height;;
            if (this.$nodeData[id].type == 'node') {
                width = this.$nodeData[id].width - hack;
                height = this.$nodeData[id].height - hack;
                if (width < 86) {
                    width = 86;
                }
                if (height < 24) {
    ...
    resizeNode: function (id, width, height) {
        ...
        if (this.$nodeData[id].type == 'node') {
            var w = width;
            var h = height;
            var top = (h - $("#nodeName_" + this.$nodeData[id].id).height()) / 2;
            var left = (w - $("#nodeName_" + this.$nodeData[id].id).width()) / 2;
            this.$nodeDom[id].children("table").css({ width: width - 2 + "px", height: height - 2 + "px" });
            $("#nodeName_" + this.$nodeData[id].id).css({ "top": "" + top + "px", "left": "" + left + "px" });
            $("#lingxing_top_" + this.$nodeData[id].id).css({ "border-bottom": "" + (height / 2.5) + "px solid #FC8F3C", "border-left": "" + (width / 2) + "px solid Transparent", "border-right": "" + (width / 2) + "px solid Transparent" });
            $("#lingxing_bottom_" + this.$nodeData[id].id).css({ "border-top": "" + (height / 2.5) + "px solid #FC8F3C", "border-left": "" + (width / 2) + "px solid Transparent", "border-right": "" + (width / 2) + "px solid Transparent" });
        }
    ...
    loadData: function (data) {
        ...
        this.$editable = t;
        this.$deletedItem = {};
        //如果是呈现界面 初始化节点状态：如是否可以点击 鼠标移动时的cursor；
        if (this.isShowPager == true) {
            $(".GooFlow_item table tr td").css({ 'cursor': "default" });
            $(".GooFlow_item table tr td b").each(function (i, ele) {
                if (this.className.indexOf('task') > -1 || this.className.indexOf('chat') > -1) {
                    $(this).parent().parent().find('td').css({ 'cursor': "pointer" });
                }
            });
            $("#draw_flow g").css({ 'cursor': "default" });
        }
    },
    //用AJAX方式，远程读取一组数据
