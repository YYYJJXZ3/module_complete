...
(function ($) {
    var methods = {
        init: function (settings) {
            ...
            if ($txtDate.parent().is("#divTimeCtrl")) {
                ...
                $div = $("<div id='divTimeCtrl'></div>"); 
                $div.css("display", "table"); $txtDate.after($div);
                $div.append($txtDate);
            }
        ...
        getDateStr: function () {
            if (this.data("range") == true) {
                return this.siblings("[id^=txtStartDate]").val() + ":" + this.val();
            }
            else {
                return this.val();
            }
        ...
        getHourStr: function () {
            ...
            if ($(this).parent().find("select[id=ddlTimeType]").length > 0) {
                ...
                tmtp = $(this).parent().find("ul[id=ddlTimeType]").find("li[class=selected]").eq(0).attr("valueli");
            }
            if (tmtp == "Hour" || tmtp.indexOf("Minute")>-1) {
        ...
        getMinuteStr: function () {
            ...
            if (tmtp.indexOf("Minute") > -1) {
                if (this.data("range") == true) {
                    return this.parent().find("#ddlStartMin").val() + ":" + this.parent().find("#ddlminute").val();
                }
                else {
                    return this.parent().find("#ddlminute").val();
                }
            }
            else {
                return "";
    ...
    $.fn.timepicker.defaults = {
        ...
        showTypeMinute5: false,
        showTypeMinute15: false,
    ...
    function initDate(ddlTimeType, $txtDate, paras) {
        ...
        var txt = $txtDate; $("#ddlhour", txt.parent()).remove(); $("#ddlminute", txt.parent()).remove(); $("#tp_txtHour", txt.parent()).hide();
        $("#ddlStartHour", txt.parent()).remove(); $("#ddlStartMin", txt.parent()).remove();
        ...
        if (paras.range == true) {
            if (!txt.prev().is("span")) {
                ...
                txtStart = txt.prevAll("[id^=txtStartDate]");
        ...
        if (timetype == "Month") {
            ...
            initTimeMonthText(txt, $divYearMonth,paras);
            if (paras.range == true) {
                txtStart.val(dateToString(paras.defaultDateStart, "yyyymm"));
                initTimeMonthText(txtStart, $divYearMonthStart,paras);
            }
        }
            ...
            initTimeWeekText(txt, $divYearMonth,paras);
            if (paras.range == true) {
                txtStart.val(dateToString(paras.defaultDateStart, "yyyyww"));
                initTimeWeekText(txtStart, $divYearMonthStart,paras);
            }
        }
        else {
            txt.val(dateToString(paras.defaultDate, "yyyymmdd"));
            paras.datePickerOptions["defaultDate"] = paras.defaultDate;
            ...
            if (timetype == "Hour" || timetype.indexOf("Minute") > -1) {
                if (timetype.indexOf("Minute") > -1) {
                    var $minuteDdl = $("<select id=ddlminute></select>");
                    var ispan = timetype == "Minute" ? 1 : (timetype == "Minute5" ? 5 : 15);
                    if (!paras.range) {
                        $minuteDdl.append($("<option value=''>全部</option>"));
                    }
                    for (var i = 0; i < 60; i += ispan) {
                        var s = i;
                        if (s < 10) {
                            s = "0" + s;
                        }
                        $minuteDdl.append($("<option value=" + s + ">" + s + "分</option>"));
                    }
                    if (paras.defaultMinuteStr != null && paras.defaultMinuteStr != "") {
                        $minuteDdl.val(paras.defaultMinuteStr.substr(0, 2));
                    }
                    txt.after($minuteDdl);
                    if (paras.range == true) {
                        var $startMinDdl = $minuteDdl.clone();
                        $startMinDdl.attr("id", "ddlStartMin");
                        $minuteDdl.val("00");
                        txtStart.after($startMinDdl);
                    }
                }
        ...
        if (paras.inline==false&&(timetype == "Month" || timetype == "Week" || timetype == "Quarter" || timetype == "Year" || timetype == "HalfYear")) {
        ...
        if(paras.inline){
        	$divYearMonth.show();
        	$divYearMonth.css("position","relative");
        }
    ...
    function initTimeHalfYearText(txt, $divYearMonth) {
        ...
        txt.bind("click", function (event) {
            $divYearMonth.addClass("ui-datepicker").addClass("ui-widget-content");
            setDivBelowPos($divYearMonth,txt);
            $divYearMonth.show();
    ...
    function initTimeMonthText(txt, $divYearMonth,paras) {
        $divYearMonth.width(200);
        $divYearMonth.css("position", "absolute");
        $divYearMonth.css("z-index", "999");
        txt.parent().append($divYearMonth);
        if(paras.inline){
        	bindYearMonth(txt, $divYearMonth,paras);
        }
        else{
	        txt.bind("click", function (event) {
	            $divYearMonth.addClass("ui-datepicker").addClass("ui-widget-content");
    ...
    function setDivBelowPos($divBelow,txt){
    	var top = txt.offset().top;
        var left = txt.offset().left;
        $divBelow.offset({ top: top+txt.height(), left: left });
    }

    function initTimeWeekText(txt, $divYearMonth,paras) {
        $divYearMonth.width(300);
        $divYearMonth.css("position", "absolute");
        $divYearMonth.css("z-index", "999");
        txt.parent().append($divYearMonth);
        if(paras.inline){
        	bindYearWeek(txt, $divYearMonth,paras);
        }
        else{
	        txt.bind("click", function (event) {
	            $divYearMonth.addClass("ui-datepicker").addClass("ui-widget-content");
    ...
    //绑定年月
    function bindYearMonth(txt, $divYearMonth,paras) {
        $divYearMonth.empty();
        addPreNextButton(txt, $divYearMonth, bindYearMonth,paras);
        var $ulYearMonth = $("<table class='ui-datepicker-calendar' style='width:200px;'></table>");
        var date = new Date();
        var curMonth = date.getMonth();
        var yy = date.getFullYear() + year;
        for (var r = 0; r < 4; r++) {
            ...
            for (var i = 1; i <= 3; i++) {
                ...
                option.click(function () {
                    txt.val(yy + "年" + $(this).text());
                    if(paras.onSelect!=null){
                    	paras.onSelect(yy + "年" + $(this).text());
                    }
                    if(!paras.inline){
                    	$divYearMonth.empty();
                    }
    ...
    //绑定年周
    function bindYearWeek(txt, $divYearWeek,paras) {
        $divYearWeek.empty();
        addPreNextButton(txt, $divYearWeek, bindYearWeek,paras);
    ...
    function addPreNextButton(txt, $divYearMonth, func,paras) {
        ...
        var $div = $("<tr></tr>");
        var $div1 = $("<td class='ui-datepicker-prev' style='border:0px'></td>");
        var $div2 = $("<td class='ui-datepicker-next' style='border:0px' align='right'></td>");
        var $btnPre = $("<span class='ui-icon ui-icon-circle-triangle-w' title='上一年' style='width:12px;border:0px'></span>");
        var $td = $("<td align='center' class='ui-datepicker-title' valign='middle'></td>");
        $btnPre.click(function () {
            year = year - 1;
            func(txt, $divYearMonth,paras);
    ...
    */
    ...
    function AddDdlTimeType(paras) {
        ...
        if (paras.isTile) {
            ...
            if (paras.showTypeMinute15) {
                s += "<li valueli=Minute15>15分钟</li>";
            }
            if (paras.showTypeMinute5) {
                s += "<li valueli=Minute5>5分钟</li>";
            }
            ...
            if (paras.showTypeMinute15) {
                s += "<option value=Minute15>15分钟</option>";
            }
            if (paras.showTypeMinute5) {
                s += "<option value=Minute5>5分钟</option>";
            }if (paras.showTypeMinute) {
